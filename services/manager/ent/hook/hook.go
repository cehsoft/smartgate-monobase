...
func (f ContainerTrackingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ContainerTrackingMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ContainerTrackingMutation", m)
	}
	return f(ctx, mv)
}
